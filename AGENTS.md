...
Step 3 (property computation) has started: `src/chem/partial-charges.ts` implements an eight-pass, topology-only Gasteiger-Marsili / PEOE partial-charge model with hybridization-dependent parameters. It initializes from formal charges, applies simultaneous per-bond transfers, and conserves each disconnected component's formal charge exactly. `src/chem/geometry.ts` builds a deterministic constrained display conformer from tabulated and resonance-adjusted bond lengths (including aromatic rings embedded in substituted structures), sp/sp2/sp3 angles (with lone-pair compression), stored tetrahedral parity, double-bond bond-pair geometry, coplanar conjugated systems and explicit non-intersection checks; coordinates are Three.js `Vector3` values and chemistry-independent vector/plane/constraint operations live in `src/math/vector3.ts`. Large explicit-H molecules settle their heavy skeleton before anchored hydrogen placement, `geometryIssues` tests every demo source (including beta-carotene), and the registry caches each generated conformer with mutation-aware invalidation. The light-themed interactive Three.js molecule viewer renders ball-and-stick or overlapping van der Waals space-filling models and applies Structure, Hybridization, Charge, qualitative Electron cloud (bond clouds, VSEPR/localized lone pairs and merged conjugated π clouds) and merged, color-grouped pi-orbital overlays directly to the 3D atoms; the two systems on a triple bond are coordinated 90 degrees apart, opposite phase surfaces barely touch, bounds include the complete metaball field, and hovering a π surface shows its ID, atom count and electron count beside the pointer. Marching-cubes resolution is tied to a maximum physical cell width so long systems stay smooth and connected, and the opposite lobe mirrors the first lobe's geometry instead of rerunning the scalar field. There is no numbered atom list. HOMO/LUMO energies and wavefunctions and the remaining property calculators are still open; the pi-orbital layer visualizes perceived conjugated p orbitals, not a computed HOMO/LUMO.
