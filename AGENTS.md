...
- `src/chem/` - chemistry engine (molecule data structure on graphology; canonical naming, properties, thermodynamics next; `registry.ts` global SMILES-to-graph map + lazy structure-diagram SVG cache, `smiles-writer.ts` game graph -> SMILES writer and `rdkit.ts` WASM loader behind `Molecule.getRdkitMolecule`).
