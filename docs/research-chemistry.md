...
The localized σ calculation now follows the qualitative extended-Hückel/LCAO construction: a 2x2 Hamiltonian uses the negative PEOE neutral orbital electronegativities as its diagonal atomic energies and a fixed -4 eV interaction. Diagonalization therefore makes a heteronuclear bonding mode larger on the lower-energy (more electronegative) atom and its orthogonal antibonding partner larger on the other atom. This is the inexpensive two-center version of the coefficient/electronegativity relationship developed by Mulliken (https://doi.org/10.1063/1.1749731) and the σ-inclusive extended Hückel approach of Hoffmann (https://doi.org/10.1063/1.1734456); overlap is still deliberately omitted for game-scale cost.

Integer charge now shifts every atomic orbital basis level using the same PEOE polynomial as the charge model, `chi(q) = a + b q + c q^2`: `Delta E_formal = -0.87 [chi(f) - chi(0)]`, where `f` is formal charge and 0.87 approximates molecular relaxation/screening. This replaces the earlier tiny linear response that left anionic electrons unrealistically deep. The polynomial comes from Gasteiger and Marsili's charge-dependent orbital-electronegativity method (https://doi.org/10.1016/0040-4020(80)80168-2). The screening factor is calibrated to alkoxide photoelectron data: ethoxide's measured adiabatic electron affinity is 1.712 +/- 0.004 eV (https://doi.org/10.1063/1.480767), with a later series giving EA 1.710 +/- 0.020 eV and vertical detachment 1.755 eV (https://doi.org/10.1016/j.cplett.2019.136638). The game predicts -1.67 eV for the detachable ethoxide lone pair instead of -11.70 eV; neutral ethanol remains -12.06 eV. This is an empirical topology model, so reaction code should consume trends and frontier ordering rather than treat every displayed level as a laboratory observable.
