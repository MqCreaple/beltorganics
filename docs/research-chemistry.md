...
`src/chem/elements.ts` is the single source of truth for H/B/C/N/O/F/Cl/Br/I: invented name and root, atomic number, period, default and allowed valence, valence-electron count, electron configuration, lone pairs, greediness, covalent and van der Waals radii, display color, PEOE fits and Hückel on-site shift. Geometry, charge, SMILES import and visualization consume this catalog instead of maintaining element switches. RDKit's documented allowed-valence table is the compatibility baseline: https://github.com/rdkit/rdkit/blob/master/Docs/Book/RDKit_Book.rst. PEOE fits come from RDKit's primary parameter source: https://github.com/rdkit/rdkit/blob/master/Code/GraphMol/PartialCharges/GasteigerParams.cpp. Covalent radii are conventional single-bond estimates and main-group van der Waals radii follow Mantina et al., https://doi.org/10.1021/jp8111556. S/P were removed because a naive octet/VSEPR treatment gives wrong lone pairs and geometry for sulfone and sulfonic-acid structures; Li/Mg were removed because the current SMILES path does not support the required organometallic representation. All four remain roadmap candidates.
...
- Parameters live on each `ElementInfo`, including hybridization fits for C/N/O/B and fits for F/Cl/Br/I. The expanded values follow RDKit's Gasteiger parameter source (`Code/GraphMol/PartialCharges/GasteigerParams.cpp`). Keeping the table with the radii, valences, electron configuration and visualization color prevents element behavior from drifting across subsystems.
...
`molecularOrbitals` combines and sorts the modes and returns occupied/unoccupied lists, HOMO, LUMO and gap. Internal dimensionless levels are mapped affinely to an approximate display scale, `E_eV = -7.5 + 2.5 E_game`; this preserves ordering while giving the inspector a readable numbered eV axis. It is not an ab-initio prediction. The signed coefficient map is the important shape output: regression tests recover ethene and benzene mode spacing, the shrinking π gap from ethene to butadiene, and a carbonyl π* coefficient whose magnitude is larger on Cardinium than Obligium. The viewer's complete MO diagram selects individual σ, π and lone-pair modes; their 3D lobes show phase and coefficient magnitude.

The localized σ calculation now follows the qualitative extended-Hückel/LCAO construction: a 2x2 Hamiltonian uses the negative PEOE neutral orbital electronegativities as its diagonal atomic energies and a -7 eV interaction (with another -1 eV for each additional bond order). Bonds exchanged by the compact resonance rules used for display geometry receive the shared effective order 1.5, so raw Kekulé single/double placement does not split benzene's equivalent σ levels or the two acetate C-O couplings. Diagonalization makes a heteronuclear bonding mode larger on the lower-energy (more electronegative) atom and its orthogonal antibonding partner larger on the other atom. This is the inexpensive two-center version of the coefficient/electronegativity relationship developed by Mulliken (https://doi.org/10.1063/1.1749731) and the σ-inclusive extended Hückel approach of Hoffmann (https://doi.org/10.1063/1.1734456); overlap is still deliberately omitted for game-scale cost.

Integer charge is not added to every atomic-orbital diagonal: partial charge already changes those levels, so doing both gives the same charge two independent stabilizations and can put C-H σ above an oxygen lone pair. Instead, the PEOE polynomial `chi(q) = a + b q + c q^2` supplies one localized frontier correction, `Delta E_charge = -0.65 [chi(f) - chi(0)]`. An extra anionic lone pair receives it directly; in a conjugated ion it is distributed across the HOMO in proportion to the charge-bearing atom's coefficient. Empty orbitals receive a separate whole-ion electron-addition penalty, which prevents an anion's LUMO from falling below its occupied frontier. The polynomial comes from Gasteiger and Marsili's charge-dependent orbital-electronegativity method (https://doi.org/10.1016/0040-4020(80)80168-2).

The correction is calibrated against gas-phase photoelectron data. Ethoxide has measured adiabatic electron affinity 1.712 +/- 0.004 eV (https://doi.org/10.1063/1.480767), with a later series reporting EA 1.710 +/- 0.020 eV and vertical detachment 1.755 eV (https://doi.org/10.1016/j.cplett.2019.136638); the game predicts its detachable lone pair at -1.71 eV. Acetate's high-resolution measured electron affinity is 3.2528 +/- 0.0010 eV (https://doi.org/10.1063/1.4913924); the game predicts its delocalized π HOMO at -3.60 eV.

Frontier-character regressions cover water (n/σ*), ethanol (n/σ*), acetone (n/π*), ammonia (n/σ*), acetic acid (n/π*) and acetate (π/π*). The experimental anchors are the NBS photoionization study of water and ammonia (https://pmc.ncbi.nlm.nih.gov/articles/PMC6624710/), ethanol VUV spectroscopy assigning the oxygen nonbonding HOMO and low n -> σ* excitation (https://doi.org/10.1016/j.jqsrt.2019.106609), acetone photoelectron work identifying its oxygen-lone-pair HOMO (https://pmc.ncbi.nlm.nih.gov/articles/PMC5608382/) together with the calculated carbonyl n -> π* frontier transition (https://doi.org/10.1063/1.2992049), and high-resolution acetic-acid spectroscopy with ab-initio band assignments (https://doi.org/10.1016/j.chemphys.2005.10.032). These are qualitative topology benchmarks, not ab-initio claims: reaction code should consume the ordering and trends rather than treat every displayed level as a laboratory observable.
...
3. Canonical name format: research (section 7) suggests parity-based stereo tokens layered on the canonical graph string - SMILES-style `@`/`@@` and `/`/`\`, or an InChI-like `/t` `/b` layer - with human-readable pseudo-IUPAC as an alternative. Decide in roadmap step 2. The SMILES-style format is now implemented: `src/chem/smiles.ts` provides `toSmiles`/`parseSmiles` over RDKit.js (`docs/research-game.md` section 10; `docs/smiles-naming.md`), with verified sample outputs. Chosen convention: canonical names are RDKit's canonical SMILES (ethanol `CCO`). `TetrahedralStereo` is directly a four-incident-bond tuple in an arbitrary order under a fixed counterclockwise convention; an odd permutation is the mirror image and an absent field means unspecified. A specified double bond stores two substituent-bond ids known to be cis; equal tuple membership means any cross-end pair is cis and unequal membership means trans. This local representation avoids side-chain ranking and supports fully substituted polyenes such as beta-carotene. The player docs (`docs/01-molecules.typ`) promise `CCO`/`COC`, which is exactly RDKit's flavour. RDKit's canonicalization derives tetrahedral tokens correctly; the game's writer emits `@`/`@@` and a constraint-consistent set of `/`/`\` tokens for its serialization order, then lets RDKit canonicalize.
