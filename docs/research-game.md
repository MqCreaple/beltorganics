...
- The π-orbital overlay renders one selected mode from the Hückel solver. Signed eigenvector coefficients control phase, lobe size and nodes, while the selector exposes occupancy and relative energy. The general electron-cloud layer remains a qualitative domain display rather than a quantum density isosurface.
